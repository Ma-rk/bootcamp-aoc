...
(defn board-size->grids
  ...
  output ex) '({:grid-x 0, :grid-y 0} {:grid-x 1, :grid-y 0} {:grid-x 2, :grid-y 0} {:grid-x 3, :grid-y 0} {:grid-x 4, :grid-y 0} {:grid-x 5, :grid-y 0} {:grid-x 6, :grid-y 0} {:grid-x 7, :grid-y 0} {:grid-x 8, :grid-y 0} {:grid-x 0, :grid-y 1} {:grid-x 1, :grid-y 1} ... )"
  [board-size]
...
(defn get-dist
  ...
  input ex)
  - grid: {:grid-x 8, :grid-y 3}
  - point {:pt-x 306, :pt-y 102}
  output ex) 397"
  [grid point]
  (let [diff-x (Math/abs (- (grid :grid-x) (point :pt-x)))
        diff-y (Math/abs (- (grid :grid-y) (point :pt-y)))]
    (+ diff-x diff-y)))
...
(defn mark-nearest-point-on-grid
  ...
  input ex) '({:grid-x 0, :grid-y 0} {:grid-x 1, :grid-y 0} {:grid-x 2, :grid-y 0} {:grid-x 3, :grid-y 0} {:grid-x 4, :grid-y 0} {:grid-x 5, :grid-y 0} {:grid-x 6, :grid-y 0} {:grid-x 7, :grid-y 0} {:grid-x 8, :grid-y 0} {:grid-x 0, :grid-y 1} {:grid-x 1, :grid-y 1} ... )
  output ex) '{:grid-x 4, :grid-y 0, :nearest-pt {:pt-x 1, :pt-y 1}} {:grid-x 5, :grid-y 0, :nearest-pt :tied} {:grid-x 6, :grid-y 0, :nearest-pt {:pt-x 8, :pt-y 3}} ...)"
  [grids points]
...
(defn collect-infitite-points
  ...
  input ex) '{:grid-x 4, :grid-y 0, :nearest-pt {:pt-x 1, :pt-y 1}} {:grid-x 5, :grid-y 0, :nearest-pt :tied} {:grid-x 6, :grid-y 0, :nearest-pt {:pt-x 8, :pt-y 3}} ...) 
  output ex) '({:pt-x 1, :pt-y 1} {:pt-x 8, :pt-y 3} {:pt-x 1, :pt-y 6} {:pt-x 8, :pt-y 9})"
  [marked-grids]
  (let [max-x (apply max (map :grid-x marked-grids))
        max-y (apply max (map :grid-y marked-grids))
        edge-reached-grids (filter (fn [grid] (or (= (:grid-x grid) max-x)
                                                  (= (:grid-y grid) max-y)
                                                  (= (:grid-x grid) 0)
                                                  (= (:grid-y grid) 0))) marked-grids)
...
(comment
  (let [points (-> "src/input/aoc2018_6_input.txt"
        ...
        count-of-finite-points (->> marked-grids
                                    ...
                                    (get-count-of-each-finite-points marked-grids))]
    (val (first (sort-by val > count-of-finite-points))))
  (class [{:pt-x 5, :pt-y 5} 17]))
;; 파트 2
...
(defn get-count-of-dist-under-10000
  ...
  input ex) ({:grid-x 0, :grid-y 0, :dist 20335} {:grid-x 1, :grid-y 0, :dist 20285} {:grid-x 2, :grid-y 0, :dist 20235} {:grid-x 3, :grid-y 0, :dist 20185} ...)
  output ex) 15"
  [dist-appended-grids]
